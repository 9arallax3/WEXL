<script>
    ...
    import {randomClock, rot_degree, min_phrases} from './jsfiles/clock.js';
    ...
    function tell_time() {
        ...
        if (clockObj.hours == 0)
            clockObj.hours =12;
        switch(clockObj.minutes) {
            case 0:
                return (str + toWord.toWords(clockObj.hours) + min_phrases[0]);
                break;
            case 5:
                return (str + min_phrases[1] + toWord.toWords(clockObj.hours)+".");
                break;
            case 10:
                return (str + min_phrases[2] + toWord.toWords(clockObj.hours)+".");
                break;
            case 15:
                return (str + min_phrases[3] + toWord.toWords(clockObj.hours)+".");
                break;
            case 20:
                return (str + min_phrases[4] + toWord.toWords(clockObj.hours)+".");
                break;
            case 25:
                return (str + min_phrases[5] + toWord.toWords(clockObj.hours)+".");
                break;
            case 30:
                return (str + min_phrases[6] + toWord.toWords(clockObj.hours)+".");
                break;
            case 35:
                return (str + min_phrases[7] + toWord.toWords(clockObj.hours+1)+".");
                break;
            case 40:
                return (str + min_phrases[8] + toWord.toWords(clockObj.hours+1)+".");
                break;
            case 45:
                return (str + min_phrases[9] + toWord.toWords(clockObj.hours+1)+".");
                break;
            case 50:
                return (str + min_phrases[10] + toWord.toWords(clockObj.hours+1)+".");
                break;
            case 55:
                return (str + min_phrases[11] + toWord.toWords(clockObj.hours+1)+".");
                break;
            default:
                return (str + "time to take a break! ;-)")
        };
    };
...
<div class="stack-large">
    <p style="color:gray">Practice telling time by using phrases: </p>
            ...
            <div>    
                {#if ans_submit}
                    {#if (hours == clockObj.hours)}
                        ...
                        {/if}
                    {:else if (hours == 12) && (clockObj.hours == 0)}
                        {#if (minutes == clockObj.minutes)}
                            <p>{c_ans.CORRECT_ANS_MSG[c_ans_rn].msg}</p>
                        {:else}
                            <p>The "hour" value is correct, but the "minutes" value is not correct!</p>
